...
using GPSReader.USB.Classes.DLL;

namespace GPSReader.USB.Classes
{
    public static class DeviceDiscovery
    {
        ...
        /// 查找所有USB外设
        /// </summary>
        /// <param name="listOfDevicePathNames">USB外设路径</param>
        /// <param name="numberOfDevicesFound">查找到的USB外设数量</param>
        /// <returns></returns>
        public static bool FindHidDevices(ref string[] listOfDevicePathNames, ref int numberOfDevicesFound)
        {
            ...
            bool deviceFound;
            var deviceInfoSet = new IntPtr();//C#中的IntPtr类型称为“平台特定的整数类型”，它们用于本机资源，如窗口句柄。
            ...
            //第一步，Win API 获得设备类GUID
            Hid.HidD_GetHidGuid(ref systemHidGuid);//ref 引用传递
        ...
        /// <summary>
        /// 根据待查找USB外设信息，查找该USB外设!!!!!!
        /// </summary>
        /// <param name="deviceInformation">待查找的USB外设</param>
        /// <returns></returns>
        public static bool FindTargetDevice(ref DeviceInformationStructure deviceInformation)
        {
            ...
            try
            {
                ...
                //如果匹配到 待查找USB设备， 需要得到该设备的更多信息， 然后 打开读写句柄 允许PC与USB外设通信
                if (isDeviceDetected)
                {
                    ...
                    //
                    //查询设备的性能（主要是输入输出报的大小，验证 设备 发送或者接收的信息 没有超过设备的性能限制）
                    //
        ...
        /// 根据USB外设，查询该设备的信息
        /// </summary>
        /// <param name="deviceInformation">USB外设</param>
        public static void QueryDeviceCapabilities(ref DeviceInformationStructure deviceInformation)
        {
            ...
            try
            {
                ...
                Debug.WriteLine("usbGenericHidCommunication:queryDeviceCapabilities() ->     Input Report Byte Length: {0}",
                                              deviceInformation.Capabilities.InputReportByteLength);//输入缓冲区大小
